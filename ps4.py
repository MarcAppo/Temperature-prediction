...
def generate_models(x, y, degs):
    ...
    result = []
    for deg in degs:
        result.append(np.array(np.polyfit(x, y, deg)))
    return result
...
def r_squared(y, estimated):
    ...
    """
    observed, predicted = np.array(y), np.array(estimated)
...
def evaluate_models_on_training(x, y, models):
    ...
    """
    for model in models:
        xVals, yObserved, yPredicted = x, y, np.polyval(model, x)
        pylab.plot(xVals, yObserved, 'bo', label = 'Observed temperatures')
        pylab.plot(xVals, yPredicted, 'r-', label = 'Predicted temperatures')
        rSquared = r_squared(yObserved, yPredicted)
        pylab.title('Model: polynom of degree ' + str(len(model)-1) + '\n'
                   + 'R_squared = ' + str(round(rSquared, 5)))
        pylab.xlabel('Years')
        pylab.ylabel('Temperatures in C')
